...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on("online-users", (onlineUser) => {
      ...
      setOnlineUser(onlineUser.map(user => ({
        ...user,
        username: user.name,
        _id: user.userId
      })));
    ...
    socket.on("callAccepted", (data) => {
      ...
      if (connnectionRef.current) {
        ...
        connnectionRef.current.signal(data.signal);
      }
    });
  ...
  const allusers = async () => {
    try {
      ...
      if (response.data.success !== false) {
        ...
        if (user && !fetchedUsers.find((u) => u._id === user._id)) {
  ...
  const startCall = async () => {
    try {
      if (!showReciverDetails) {
        console.error("No receiver details available");
        return;
      }
      ...
      console.log("Calling to:", showReciverDetails._id);
  ...
  const endCallCleanup = () => {
    ...
    if (connnectionRef.current) {
      connnectionRef.current.destroy();
    }
        ...
        <div className="relative w-full h-screen bg-black flex items-center justify-center">
          ...
          <div className="absolute bottom-[75px] md:bottom-0 right-1 bg-gray-900 rounded-lg overflow-hidden shadow-lg">
            <video
              ...
              playsInline
              muted
              className="w-32 h-40 md:h-52 object-cover rounded-lg"
            />
          </div>
          ...
          <div className="absolute bottom-4 w-full flex justify-center gap-4">
            ...
            <button
              onClick={toggleMic}
              className={`p-4 rounded-full text-white shadow-lg cursor-pointer ${
